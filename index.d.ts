...
export interface ChatCompletionStreamChoiceDelta extends JsonRecord {
  delta?: {
    role?: ChatRole
    content?: string
  }
...
export interface StreamErrorPayload extends JsonRecord {
  message: string
  type?: string
  code?: string | null
  param?: string | null
...
export type ResponseCreateParams = JsonRecord & {
  model: string
  input?: ResponseInput
  instructions?: string
  metadata?: Dictionary
...
export interface OpenAIHelpers {
  ...
  postStream: (path: string, data?: JsonObject) => Promise<ResponseStream>
  postForm: <TResponse>(path: string, form: FormData, parser: (input: string) => TResponse) => Promise<TResponse>
